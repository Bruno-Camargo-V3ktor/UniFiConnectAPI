...
use rocket::response::{ Redirect, status::{BadRequest, NotFound, Custom} };
...
pub async fn guest_connection_request( cookies: &CookieJar<'_>, unifi: &State< Arc< Mutex<UnifiController> > >, guest_data: Json<GuestData> ) -> Result< Status, Custom< Json<Error> > > {
    ...
    match guest_data {
        ...
        GuestData::Info( guest_info ) => {
            ...
            let r = unifi.authorize_guest( &guest_info.site, &guest_info.mac, &guest_info.minutes ).await;
